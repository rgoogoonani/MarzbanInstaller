...
getCommandOutput("unzip MainFiles.zip","utf-8")
...
getCommandOutput('echo "nameserver 178.22.122.100" > /etc/resolv.conf',"utf-8")
getCommandOutput("pip3 install -r "+Mname+"/requirements.txt","utf-8")
getCommandOutput('echo "nameserver 1.1.1.1" > /etc/resolv.conf',"utf-8")
getCommandOutput("cd "+Mname+" && alembic upgrade head","utf-8")
...
with open("/etc/systemd/system/Marzban"+Mname+".service","w") as f:
    ...
    f.close()
getCommandOutput("systemctl daemon-reload","utf-8")
getCommandOutput("systemctl start Marzban"+Mname,"utf-8")
getCommandOutput("systemctl enable Marzban"+Mname,"utf-8")
